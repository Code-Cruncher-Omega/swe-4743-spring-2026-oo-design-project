...
public class HeatingState implements ThermostatState {
    ...
    public DeviceResult execute(Thermostat context, ThermostatAction action, int tickRate) {
        ...
        return new DeviceResult(false, action.toString(), "Improper action for heating ambience."); // Invalid action for this method
    }
    @Override
        ...
        switch (action) {
            ...
            case UPDATE_STATE:
                ...
                if(newStateType.equals(getStateType())) {
                    return new DeviceResult(true, "THERMOSTAT_STILL_HEATING", context.getName() + " continues heating.");
                } else {
